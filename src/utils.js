...
export function calculateNextRecurrenceDate(currentDate, template) {
    try {
        ...
        if (template.recurrence_frequency === 'weekly' || template.recurrence_frequency === 'bi-weekly') {
            ...
            if (!template.recurrence_days) return null;
            const daysOfWeek = template.recurrence_days.split(',').map(Number).sort();
            const currentDayOfWeek = tempDate.getDay() === 0 ? 7 : tempDate.getDay(); // 1=Mon..7=Sun
            
            let nextDayFound = false;
            // Find next day in same week
            for (const day of daysOfWeek) {
                if (day > currentDayOfWeek) {
                    tempDate.setDate(tempDate.getDate() + (day - currentDayOfWeek));
                    nextDayFound = true;
                    break;
                }
            }
...
export function generateFutureOccurrences(templates, viewStartDate, viewEndDate) {
    const virtualTasks = [];
    ...
    templates.forEach(template => {
        ...
        const recurrenceEndDate = template.recurrence_ends_on ? parseDate(template.recurrence_ends_on) : limitDate;
        ...
        while (currentDate <= viewEndDate && currentDate <= recurrenceEndDate) {
            // If the current date is within the view, add it as a virtual task
            if (currentDate >= viewStartDate) {
                const deadlineDate = new Date(currentDate);
                const deadlineDays = parseInt(template.deadline_days_after_start, 10);
                ...
                virtualTasks.push({
                    ...
                    id: `virtual-${template.id}-${formatDate(currentDate)}`,
            ...
            const nextOccurrenceDate = calculateNextRecurrenceDate(currentDate, template);
...
export function generateGCalUrl(task) {
    ...
    const descriptionText = task.description ? task.description.replace(/<[^>]*>?/gm, '') : '';
