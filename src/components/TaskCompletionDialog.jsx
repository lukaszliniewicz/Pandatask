...
const CompletionWorkBreakdownFields = React.lazy(() =>
  import("./work/CompletionWorkBreakdownFields"),
);

const TaskCompletionDialog = ({ task, changeComment = "", onClose }) => {
  ...
  useEffect(() => {
    ...
    setWorkItems([]);
    setResidual({ activity_type: "", capacity: "" });
    setError("");
  }, [task?.id]);
  ...
  const submit = async (event) => {
    ...
    setError("");
    try {
      const { serializeCompletionWorkItems, serializeResidualClassification } =
        await import("./work/completionWorkModel.mjs");
      await completeTask.mutateAsync({
        ...
        workItems:
          noPersonalWork || notTracked
            ? []
            : serializeCompletionWorkItems(
                workItems,
                actualSeconds - specificSeconds,
              ),
        residual:
          noPersonalWork || notTracked
            ? {}
            : serializeResidualClassification(residual),
      });
      onClose();
    } catch (err) {
      setError(
