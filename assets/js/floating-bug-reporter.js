(function () {
    ...
    const loadScript = (src) => new Promise((resolve, reject) => {
        if (window.Pandatask && typeof window.Pandatask.mountFloatingBugReporter === 'function') {
            resolve();
            return;
        }

        const existing = document.querySelector('script[data-pandatask-lazy="full-script"]');
        if (existing) {
            existing.addEventListener('load', () => resolve(), { once: true });
            existing.addEventListener('error', () => reject(new Error('Unable to load Pandatask reporter.')), { once: true });
            return;
        }

        if (!src) {
            reject(new Error('Pandatask reporter script URL is missing.'));
            return;
        }
        ...
        script.addEventListener('error', () => reject(new Error('Unable to load Pandatask reporter.')), { once: true });
        document.body.appendChild(script);
    });

    const applySavedPosition = (launcher) => {
        const savedPosition = window.localStorage ? window.localStorage.getItem(positionKey) : null;
        if (!savedPosition) {
            return;
        }
    ...
    const makeDraggable = (launcher) => {
        ...
        launcher.addEventListener('pointermove', (event) => {
            ...
            moved = true;
            const rect = launcher.getBoundingClientRect();
            const x = Math.max(0, Math.min(event.clientX - pointerOffsetX, window.innerWidth - rect.width));
            const y = Math.max(0, Math.min(event.clientY - pointerOffsetY, window.innerHeight - rect.height));
    ...
    const mountFullReporter = async (root, button) => {
        ...
        try {
            ...
            if (!window.Pandatask || typeof window.Pandatask.mountFloatingBugReporter !== 'function') {
                throw new Error('Pandatask reporter mount API is unavailable.');
            }
